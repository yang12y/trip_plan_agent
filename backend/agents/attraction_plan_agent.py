...
async def init_attraction_plan_agent():
    global attraction_plan_agent
...
# 提供一个异步函数来获取agent
async def get_attraction_plan_agent():
    global attraction_plan_agent
    if attraction_plan_agent is None:
        # 在第一次调用时初始化
        await init_attraction_plan_agent()
    return attraction_plan_agent
